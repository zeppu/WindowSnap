...
namespace MMSnap
{
    internal class Program
    {
        ...
        public static void Main()
        {
            ...
            var hhook = SetWinEventHook(EVENT_SYSTEM_MOVESIZEEND, EVENT_SYSTEM_MOVESIZEEND, IntPtr.Zero,
            ...
            IntPtr hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
        {
            ...
            Console.WriteLine($"Window dropped: {position.X}x{position.Y}");
            Rectangle option = Rectangle.Empty;
            if (Between(position.X, 0, 100))
            {
                option = options.Options["QuarterLeft"];
            }
            if (Between(position.X, 500, 800))
            {
                option = options.Options["Centered"];
            }
            if (Between(position.X, 2000, 2500))
            {
